...
def start_fn():
	global path
	path = input("what is the path to the data?")#C:/Users/YISS/Desktop/data_mining/hw/hw10
	os.chdir(path)
...
def save_as_file_reg():
	with open("HW10KangJH_python_regression_output.txt","w") as txt:
		print("Coefficients", "\n", "-------------",sep="", file=txt)
...
def classification_bagging_ensemble():
	global data1, data1_x, data1_test, data1_test_x, accuracy_LDA_training, accuracy_LDA_testing, list_output_class1, list_output_class2, name, data1_raw, accuracy_LDA_testing2
	classification_LDA()
	name = "Bagging_Ensemble"

	data1_raw = np.array(data1_test).copy()
	data1_x_raw = np.delete(data1_raw, [p-1], axis=1)
	n = data1_raw.shape[0]

	list_idx = np.arange(0, n)
	df_class = np.zeros((n,1))

	list_bagging_accuracy = []
	df_classes = pd.DataFrame(np.array(np.arange(0,n).reshape(n,1)))
	list_classes_final = []
	for i in range(51):
		np.random.seed(i)
		selected_idx = np.random.choice(list_idx, size=list_idx.shape, replace=True)
		data1 = data1_raw[selected_idx, :].copy()
		data1_x = data1_x_raw[selected_idx, :].copy()
		...
		for k in range(1, K+1):
			...
			list_tempk = []
			for l in range(n_k):
				list_xk_temp = np.array(data1_k_x.iloc[l, :]).reshape(p-1, 1)
				mat7 = list_xk_temp - list_xk_bar
				temp5 = np.matmul(mat7, np.transpose(mat7))
				list_tempk.append(temp5)
			S_k = sum(list_tempk)/(n_k-1)
		...
		for j in range(len(list_S)):
			temp6 += (list_classes_cnt[j]-1)*list_S[j]
		Sp = 1/(sum(list_classes_cnt)-K)*(temp6)
		Sp_inv = np.linalg.inv(Sp)
		#testing_data
		data1 = np.array(data1)
		data1_test = np.array(data1_test)
		list_output_class1 = []
		for j in range(len(data1_x)):
			x_vec = data1_x[j, :].reshape(p-1, 1)
			list_discriminant = []
			for k in range(1, K+1):
				n_k = list_classes_cnt[k-1]
				data1_k = data1[data1[:, p-1] == k]
			...
			output_class1 = list_discriminant.index(max(list_discriminant))+1
			list_output_class1.append(output_class1)
		accuracy_LDA_testing2 = sum(data1[:, p-1] == list_output_class1)/len(data1)
		list_bagging_accuracy.append([i, accuracy_LDA_testing2])

		temp11 = np.concatenate((np.array(selected_idx).reshape(n, 1), np.array(list_output_class1).reshape(n, 1)), axis=1)
		temp12 = temp11[temp11[:,0].argsort()]
		temp13 = unique_rows(temp12)
		temp14 = np.array(pd.merge(df_classes, pd.DataFrame(temp13), how='left').iloc[:, 1]).reshape(n,1)
		list_classes_final.append(temp14)

	temp21 = np.array(list_classes_final).reshape(51, n)
	list_output_class11 = []
	for i in range(n):
		temp22 = pd.DataFrame(temp21[:, i]).groupby(0).size()
		temp23 = int(pd.DataFrame(temp22).index[np.argmax(np.array(temp22))])
		list_output_class11.append(temp23)
	list_output_class1 = list_output_class11.copy()
	accuracy_LDA_testing2 = sum(data1_raw[:, p-1] == list_output_class1)/n

def classification():
	global what, num_of_class, continuous_or_categorical
	num_of_class = len(set(np.array(data1.iloc[:, col_num])))
	if num_of_class>2:
		print("Select the classifier(1 = 'LDA' or 2 = 'QDA' or 3 = 'RDA' or 7 = 'Bagging_Ensemble')")
		what = int(input("Select the classifier(1 = 'LDA' or 2 = 'QDA' or 3 = 'RDA' or 7 = 'Bagging_Ensemble')"))
	else:
...
def get_confusion_matrix():
	...
	if what == 6:
		...
		#no bagging
		confusion_matrix_testing1= []
		for k in range(len(list_classes)):
			i = k+1
			for l in range(len(list_classes)):
				j = l+1
				confusion_matrix_testing1.append(sum((data1_raw[:, p-1]==i) & (np.array(list_output_class2)==j)))
		df_confusion_matrix_testing1 = pd.DataFrame(np.array(confusion_matrix_testing1).reshape(K,K))
		df_confusion_matrix_testing1.index = np.arange(1, len(df_confusion_matrix_testing1) + 1)
		df_confusion_matrix_testing1.columns = np.arange(1, len(df_confusion_matrix_testing1) +1)
		#bagging
		...
		for k in range(len(list_classes)):
			...
			for l in range(len(list_classes)):
				j = l+1
				confusion_matrix_testing2.append(sum((data1_raw[:, p-1]==i) & (np.array(list_output_class1)==j)))
...
def save_as_file_classification():
	with open("HW10KangJH_python_"+name+"_output.txt","w") as txt:
		if what==7:
				print("   (1)    LDA - no bagging", sep="", file=txt)
				print("ID, Actual class, LDA-nobagging pred", "\n", "-----------------------------", sep="", file=txt)
				for k in range(len(data1_test)):
					print(k+1, ", ", int(data1_raw[k, col_num]), ", ", list_output_class2[k], sep="", file=txt)
				print("", file=txt)
				print("Confusion Matrix (LDA - no bagging)", "\n", "----------------------------------", file=txt)
				print(df_confusion_matrix_testing1, file=txt)
				print("", file=txt)
				print("Model Summary (LDA - no bagging)", "\n", "------------------------------", sep="", file=txt)
				print("Overall accuracy = .", str(round(accuracy_LDA_testing, 3))[2:], "\n", sep="", file=txt)

				print("   (2)    LDA - bagging", sep="", file=txt)
				print("ID, Actual class, LDA-bagging pred", "\n", "-----------------------------", sep="", file=txt)
				for k in range(len(data1_test)):
					print(k+1, ", ", int(data1_raw[k, col_num]), ", ", list_output_class1[k], sep="", file=txt)
				print("", file=txt)
				print("Confusion Matrix (LDA - bagging)", "\n", "----------------------------------", file=txt)
				print(df_confusion_matrix_testing2, file=txt)
				print("", file=txt)
				print("Model Summary (LDA - bagging)", "\n", "------------------------------", sep="", file=txt)
				print("Overall accuracy = .", str(round(accuracy_LDA_testing2, 3))[2:], sep="", file=txt)
		else:
			if what==3:
				print("alpha = ", alpha, "\ngamma = ", gamma, "\n", sep="", file=txt)
				fig = plt.figure()

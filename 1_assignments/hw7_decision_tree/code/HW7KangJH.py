...
def start_fn():
	global path
	path = input("what is the path to the data?")#C:/Users/YISS/Desktop/data_mining/hw/hw7
	os.chdir(path)
...
def save_as_file_reg():
	with open("HW7KangJH_python_regression_output.txt","w") as txt:
		print("Coefficients", "\n", "-------------",sep="", file=txt)
...
def save_as_file_classification():
	with open("HW7KangJH_python_"+name+"_output.txt","w") as txt:
		if what==3:
			print("alpha = ", alpha, "\ngamma = ", gamma, "\n", sep="", file=txt)
			fig = plt.figure()

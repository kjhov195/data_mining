...
def start_fn():
	global path
	path = input("what is the path to the data?")#C:/Users/YISS/Desktop/data_mining/hw/hw6
	os.chdir(path)
...
def save_as_file_reg():
	with open("HW6KangJH_python_regression_output.txt","w") as txt:
		print("Coefficients", "\n", "-------------",sep="", file=txt)
...
def classification_data_preprocessing():
	global n, p, data1, data1_test, list_classes, list_classes_cnt, K, list_p, list_S, Sp, Sp_inv, data11, data11_test
	n = data1.shape[0]
	p = data1.shape[1]
...
def classification_Naivebayes():
	...
	accuracy_Naive_bayes_testing = sum(data1_y == list_output_class2)/len(data1_y)
...
def classification():
	...
	if num_of_class>2:
		...
		print("Select the classifier(1 = 'LDA' or 2 = 'QDA' or 3 = 'RDA' or 4 = 'Logistic' or 5 = 'Naive_bayes')")
		what = int(input("Select the classifier(1 = 'LDA' or 2 = 'QDA' or 3 = 'RDA' or 4 = 'Logistic' or 5 = 'Naive_bayes')"))
...
def save_as_file_classification():
	with open("HW6KangJH_python_"+name+"_output.txt","w") as txt:
		if what==3:
			print("alpha = ", alpha, "\ngamma = ", gamma, "\n", sep="", file=txt)
			fig = plt.figure()
